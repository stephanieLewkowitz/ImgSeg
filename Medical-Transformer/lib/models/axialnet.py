...
class AxialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        kr = tf.einsum('bgci,cij->bgij', k, k_embedding).transpose(2, 3)     
...
class AxialAttention_dynamic(nn.Module):
    ...
    def forward(self, x):
        ...
        all_embeddings = torch.index_select(self.relative, 1, self.flatten_index).view(self.group_planes * 2, self.kernel_size, self.kernel_size)
        q_embedding, k_embedding, v_embedding = torch.split(all_embeddings, [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=0)
        qr = tf.einsum('bgci,cij->bgij', q.cpu().detach().numpy(), q_embedding.cpu().detach().numpy())
        kr = tf.einsum('bgci,cij->bgij', k.cpu().detach().numpy(), k_embedding.cpu().detach().numpy()).transpose(2, 3)
        qk = tf.einsum('bgci, bgcj->bgij', q.cpu().detach().numpy(), k.cpu().detach().numpy())
